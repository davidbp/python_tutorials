...
cdef inline float dist_pqcode_to_codebook(long M, float[:,:] dtable,cython.integral[:] pq_code):
    cdef float dist = 0
    cdef int m
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef dist_pqcodes_to_codebooks(const long M, float[:,:] dtable, cython.integral[:,:] pq_codes):
    cdef:
        int m, loops
