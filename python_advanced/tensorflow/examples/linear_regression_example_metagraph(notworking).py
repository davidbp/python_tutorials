...
DIRECTORY_SAVERS = "./checkpoints/"
META_FILE_NAME = "linear_regression_saver.meta"
n_epochs = 20
...
with tf.Session(graph=linear_regression_graph) as sess:
    ...
    if os.path.exists(DIRECTORY_SAVERS + META_FILE_NAME ):
        print("\nCheckpoint found, keep training")
        new_saver = tf.train.import_meta_graph(DIRECTORY_SAVERS  + META_FILE_NAME)
        new_saver.restore(sess, DIRECTORY_SAVERS  + META_FILE_NAME)
        #tf.reset_default_graph()
    ...
    tf.train.export_meta_graph(DIRECTORY_SAVERS  + META_FILE_NAME)
