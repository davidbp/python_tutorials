...
__author__ = ["David Buchaca"]
__all__ = ["MutlivariateForecastingGridSearchCV"]
...
## ISSUE check_y_X 
##    problem: it assumes `y` is a `pd.Series`
##    solution: defined a `multivariate_check_y_X` that checks `y` is `pd.DataFrame`




def multivariate_check_y_X(
    y,
    X=None,
    allow_empty=False,
    allow_constant=True,
    ...
    if not isinstance(y, pd.DataFrame):
        raise TypeError(f"`y` is not an instance of {pd.DataFrame}")
...
class BaseGridSearch(BaseForecaster):
    ...
    def fit(self, y, X=None, fh=None, **fit_params):
        ...
        ----------
        y : pd.DataFrame
            Target Multivariate time series to which to fit the forecaster.
        ...
        """
        y, X = multivariate_check_y_X(y, X)
